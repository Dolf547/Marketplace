...
//FRETE
		...
		if (count($_att) > 0) {
			...
			//	$_att2 = $this->mercadolivre_model->get_shipping_free_methods($id,$_att["id"]);
				
				$n_methods = 0;
				/* foreach ($_att2 as $_att2) {                        
					
					$free_methos_rules = array();
					
					$_att3 = $this->mercadolivre_model->get_products_shipping_free_methods_rules($id,$_att2["id"]);

					foreach ($_att3 as $_att3) {   
					   $default = null;
					   if(!empty($_att3["default"])){
						 $default = (empty($_att3["default"]==0)) ? false : true;
					   }

					   $free_shipping_flag = null;
					   if(!empty($_att3["free_shipping_flag"])){
						 $free_shipping_flag = (empty($_att3["free_shipping_flag"]==0)) ? false : true;
					   }

					   $free_methos_rules = array(
							"free_mode"     => $_att3["free_mode"],
							"value"         => $_att3["value_ml"],
							"default"       => $default,
					...
					$free_methos[$n_methods]["id"] = $_att2["id_ml"];
					$free_methos[$n_methods]["rule"] = $free_methos_rules;
					$n_methods++;
 ...
 */
				$tags = array();
				$n_tag = 0;
			//	$_att2 = $this->mercadolivre_model->get_products_shipping_tags($id,$_att["id"]);

			/* 	foreach ($_att2 as $_att2) {
					$tags[$n_tag] = $_att2["id_ml"];
					$n_tag++;
				} */
